    ...
    data: new SlashCommandBuilder()
      ...
      .addSubcommandGroup((group) =>
        group
          ...
          .addSubcommand((c) =>
            c.setName("view").setDescription("View a support case")
              .addStringOption((opt) => opt.setName("id").setDescription("Case ID").setRequired(true))
          )
          .addSubcommand((c) =>
            c.setName("list").setDescription("List support cases")
              .addStringOption((opt) =>
          ...
          .addSubcommand((c) =>
            c.setName("assign").setDescription("Assign a case to a staff member")
              .addStringOption((opt) => opt.setName("id").setDescription("Case ID").setRequired(true))
              .addUserOption((opt) => opt.setName("staff").setDescription("Staff member to assign").setRequired(true))
          )
          .addSubcommand((c) =>
            c.setName("status").setDescription("Update case status")
              .addStringOption((opt) => opt.setName("id").setDescription("Case ID").setRequired(true))
              .addStringOption((opt) =>
...
async function handleTicket(interaction: ChatInputCommandInteraction, guildId: string): Promise<void> {
...
async function handleReport(interaction: ChatInputCommandInteraction, guildId: string): Promise<void> {
  const config = loadGuildConfig(guildId);
  const reportsConfig = config.reports ?? { enabled: false, requireEvidence: false, aiAnalysisEnabled: true, autoEscalateHighRisk: true };
  ...
  const replyEmbed = new EmbedBuilder()
    ...
    .setColor(0xef4444)
    .setFooter({ text: "If you have additional evidence, use /support ticket and reference this case ID." });
...
async function handleAppeal(interaction: ChatInputCommandInteraction, guildId: string): Promise<void> {
  const config = loadGuildConfig(guildId);
  const appealsConfig = config.appeals ?? { enabled: false, aiAnalysisEnabled: true };
  ...
  const replyEmbed = new EmbedBuilder()
    ...
    .setColor(0xf59e0b)
    .setFooter({ text: "If you have additional information, use /support ticket and reference this case ID." });
