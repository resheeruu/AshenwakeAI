...
export async function updatePlayer(player: GamePlayer): Promise<void> {
  const players = await loadPlayers();
    ...
    .sort((a, b) => {
      switch (category) {
        ...
        case "xp":
          return b.xp - a.xp;
        case "hunts":
          return (b.huntsCompleted ?? 0) - (a.huntsCompleted ?? 0);
        case "hunt_streak":
          return (b.bestHuntStreak ?? 0) - (a.bestHuntStreak ?? 0);
        default:
