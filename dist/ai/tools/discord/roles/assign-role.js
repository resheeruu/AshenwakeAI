...
var assign_role_exports = {};
__export(assign_role_exports, {
  createAssignRoleTool: () => createAssignRoleTool,
  executeAssignRole: () => executeAssignRole
});
module.exports = __toCommonJS(assign_role_exports);
...
function createAssignRoleTool(getClient) {
  return {
    name: "assign_role",
...
async function executeAssignRole(plan, getClient) {
  const startTime = Date.now();
  const client = getClient();
  if (!client) {
    return { status: "error", message: "Discord client is not connected." };
...
Action ID: \`${plan.id}\``,
    ...
    import_logger.logger.error(`assign_role execution failed: ${error instanceof Error ? error.message : String(error)}`);
