...
__export(delete_role_exports, {
  createDeleteRoleTool: () => createDeleteRoleTool,
  executeDeleteRole: () => executeDeleteRole
});
...
function createDeleteRoleTool(getClient) {
  return {
    name: "delete_role",
...
async function executeDeleteRole(plan, getClient) {
  const startTime = Date.now();
  const client = getClient();
  if (!client) {
    return { status: "error", message: "Discord client is not connected." };
...
Action ID: \`${plan.id}\``,
    ...
    import_logger.logger.error(`delete_role execution failed: ${error instanceof Error ? error.message : String(error)}`);
