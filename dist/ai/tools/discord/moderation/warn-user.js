...
var warn_user_exports = {};
__export(warn_user_exports, {
  createWarnUserTool: () => createWarnUserTool,
  executeWarnUserPlan: () => executeWarnUserPlan
});
module.exports = __toCommonJS(warn_user_exports);
...
function createWarnUserTool(getClient) {
  return {
    name: "warn_user",
    description: "Issue a formal warning to a member in this server.",
    category: "discord",
    ...
    execute: async (context) => {
      ...
      const reason = String(context.arguments.reason || "").trim();
      if (!userId) {
        return { status: "validation_error", message: "Missing required parameter: userId" };
      }
      if (!reason) {
        return { status: "validation_error", message: "Missing required parameter: reason" };
      }
      ...
      if (!(0, import_moderation.canModerate)(requesterMember, import_discord.PermissionFlagsBits.ModerateMembers)) {
        return { status: "denied", message: "You don't have permission to warn members.", denialReason: "MISSING_DISCORD_PERMISSION" };
      }
      ...
      const plan = (0, import_executor.createActionPlan)(
        context,
        "medium",
        [{ type: "modify", target: `@${target.user.tag}`, description: `Warn user: ${reason}` }],
        true
      );
      plan.toolName = "warn_user";
...
async function executeWarnUserPlan(plan, getClient) {
  const startTime = Date.now();
  const client = getClient();
  if (!client) {
    return { status: "error", message: "Discord client is not connected." };
  ...
  const reason = String(plan.arguments.reason || "").trim();
  if (!userId || !reason) {
    return { status: "validation_error", message: "Missing userId or reason in plan." };
  }
  ...
  const warning = (0, import_warnings.addWarning)(plan.guildId, userId, plan.requesterId, reason);
  const result = {
    status: "success",
...
Action ID: \`${plan.id}\``,
    data: { warningId: warning.id, userId, reason }
  };
