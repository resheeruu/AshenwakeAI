...
var untimeout_user_exports = {};
__export(untimeout_user_exports, {
  createUntimeoutUserTool: () => createUntimeoutUserTool,
  executeUntimeoutUserPlan: () => executeUntimeoutUserPlan
});
module.exports = __toCommonJS(untimeout_user_exports);
...
function createUntimeoutUserTool(getClient) {
  return {
    name: "untimeout_user",
...
async function executeUntimeoutUserPlan(plan, getClient) {
  const startTime = Date.now();
  const client = getClient();
  if (!client) {
    return { status: "error", message: "Discord client is not connected." };
  ...
  try {
    ...
    const result = {
      status: "success",
      message: `\u{1F50A} **Timeout removed**
**Member:** ${target}
**Reason:** ${reason}
Action ID: \`${plan.id}\``,
      data: { userId, reason }
    ...
    import_logger.logger.error(`untimeout_user execution failed: ${error instanceof Error ? error.message : String(error)}`);
    return {
      status: "error",
