...
var ban_user_exports = {};
__export(ban_user_exports, {
  createBanUserTool: () => createBanUserTool,
  executeBanUserPlan: () => executeBanUserPlan
});
module.exports = __toCommonJS(ban_user_exports);
...
function createBanUserTool(getClient) {
  return {
    name: "ban_user",
    ...
    execute: async (context) => {
      ...
      if (!botMember.permissions.has(import_discord.PermissionFlagsBits.BanMembers)) {
        return { status: "denied", message: "I don't have permission to ban members.", denialReason: "MISSING_DISCORD_PERMISSION" };
      }
      if (!(0, import_moderation.canModerate)(requesterMember, import_discord.PermissionFlagsBits.BanMembers)) {
...
async function executeBanUserPlan(plan, getClient) {
  const startTime = Date.now();
  const client = getClient();
  if (!client) {
    return { status: "error", message: "Discord client is not connected." };
...
Action ID: \`${plan.id}\``,
    ...
    import_logger.logger.error(`ban_user execution failed: ${error instanceof Error ? error.message : String(error)}`);
    return {
      status: "error",
