...
export function detectFeatureCapabilities(): FeatureCapability[] {
  ...
  return [
    ...
    { feature: "music", status: has("lavalink") && has("java") ? "available" : has("lavalink") ? "degraded" : "unavailable", reason: !has("lavalink") ? "No Lavalink" : !has("java") ? "No Java for Lavalink" : "Music ready", configurationRequired: ["LAVALINK_URL", "LAVALINK_PASSWORD"] },
    { feature: "lavalink", status: has("lavalink") ? "available" : "unavailable", reason: has("lavalink") ? "JAR + config present" : "Not found", configurationRequired: ["LAVALINK_URL", "LAVALINK_PASSWORD", "Java 21+", "FFmpeg"] },
